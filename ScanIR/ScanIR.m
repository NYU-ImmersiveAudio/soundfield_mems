...
handles.addrs = scanI2CBus(handles.a, 0);
handles.sm = stepper(handles.shield, 2, 200, 'stepType', 'Single');
handles.sm.RPM = 100;
...
elseif (plottype == 3) %time domain cascade
    ...
    maxVal = 0;
    for k = plotchls
        maxVal = max(maxVal, max(abs(handles.data(handles.app.currID).IR(:,k))));
        disp(maxVal);
    end
    yScale = .5/maxVal;
    for i = plotchls
        plot(handles.plotarea, yScale*handles.data(handles.app.currID).IR(:,i)+i);
        hold on;
    end;
...
elseif ( strcmpi (handles.specs.signalType, 'MLS') )
    ...
    y = MlsZap_selectch(handles.app.outchl,handles.app.numInputChls,handles.specs.sampleRate,mlsLen,recLen,handles.app.numPlays,.6,savewav);
elseif ( strcmpi (handles.specs.signalType, 'Golay Codes') || strcmpi (handles.specs.signalType, 'Golay-Codes'))
    ...
    y = golayZap_selectch(handles.app.outchl, handles.app.numInputChls, handles.specs.sampleRate,golayLen,recLen,handles.app.numPlays,1,.6,savewav);
end;
...
% --- Executes on button press in autoMeasureButton.
function autoMeasureButton_Callback(hObject, eventdata, handles)
% hObject    handle to autoMeasureButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

pause(30);%ample time to leave the room (GZ edit) -- timer
...
rotatePlatform(hObject,handles,rotVal);
...
% Hints: get(hObject,'String') returns contents of rotValEdit as text
%        str2double(get(hObject,'String')) returns contents of rotValEdit as a double


% --- Executes during object creation, after setting all properties.
function rotValEdit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to rotValEdit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
